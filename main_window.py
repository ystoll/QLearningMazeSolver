...
class MainWindowController:
    ...
    def TrainAgentButtonPressed(self, button):
        ...
        if self.maze.get_end_point() is None:
            self.__show_popup(
                'Error: End point not specified. Please specify end point by selecting a block and clicking "Set Endpoint"'
            )
            return
    ...
    def RunAgentButtonPressed(self, button):
        if self.agent is None or not self.agent.is_trained():
            self.__show_popup(
                "Error: Agent not trained. Please train the agent before attempting to run it"
            )
            return
        ...
        if start_point is None:
            self.__show_popup(
                'Error: Start point not specified. Please specify start point by selecting a block and clicking "Set Starting Point"'
            )
            return
    ...
    def MazeImageEventBoxPressed(self, event_box, click_event):
        ...
        self.maze_selected_block = (
            int(click_event.y // block_size),
